...
const handlePaygateWebhook = async (req, res) => {
    ...
    const { status, transaction_id, external_reference, errorCode } = payload;
    ...
    // Both single and batch payments are stored with the same transaction_id.
    // One lookup by transaction_id covers all cases.
    ...
    if (status === 'SUCCESS' && (errorCode === 0 || errorCode === '0')) {
        ...
        for (const payment of pendingPayments) {
            try {
                ...
                // Update payment with subscription_id + mark SUCCESS
