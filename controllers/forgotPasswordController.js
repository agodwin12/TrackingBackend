...
async function sendOTPViaSMS(phoneWithoutPlus, otp) {
    ...
    try {
        ...
        logger.error(`❌ [ForgotPassword] SMS API error ${response.status}:`, response.data);
        ...
        return { success: false, error: { type: err.code, message: err.message } };
    }
}

// ── Keycloak password sync (non-blocking) ─────────────────────────────────────

async function syncKeycloakPassword(user, newPassword) {
    if (!user.keycloak_id) {
        logger.warn(`⚠️ [ForgotPassword] User ${user.id} has no keycloak_id — skipping Keycloak sync`);
        return;
    }
    try {
        await keycloakService.resetKeycloakPassword(user.keycloak_id, newPassword);
        logger.info(`✅ [ForgotPassword] Keycloak password synced for user ${user.id}`);
    } catch (err) {
        // Don't fail the request — MySQL reset already succeeded
        logger.error(`❌ [ForgotPassword] Keycloak sync failed for user ${user.id}: ${err.message}`);
    }
}
...
exports.requestOTP = async (req, res) => {
    try {
        ...
        logger.info(`📲 [ForgotPassword] OTP generated for user ${user.id} — sending via SMS`);

        otpStore.set(phoneWithPlus, { otp, expiryTime, userId: user.id, attempts: 0 });

        const smsResult = await sendOTPViaSMS(normalizePhoneForSMS(phoneWithPlus), otp);
...
exports.verifyOTP = async (req, res) => {
    try {
        ...
        if (!storedOTPData) {
            return res.status(404).json({ success: false, message: 'No OTP request found. Please request a new OTP.' });
        }
        ...
        logger.info(`✅ [ForgotPassword] OTP verified for user ${storedOTPData.userId}`);
...
exports.resetPassword = async (req, res) => {
    try {
        ...
        if (!resetData) {
            return res.status(404).json({ success: false, message: 'Invalid or expired reset session. Please start over.' });
        }
        ...
        logger.info(`✅ [ForgotPassword] MySQL password reset for user ${user.id}`);

        // ── Step 2: sync to Keycloak (non-blocking) ───────────────────────────
...
exports.resendOTP = async (req, res) => {
    try {
        ...
        logger.info(`📲 [ForgotPassword] New OTP generated for user ${user.id} — sending via SMS`);
