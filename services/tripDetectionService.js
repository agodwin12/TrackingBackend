...
// Required: GOOGLE_MAPS_API_KEY must be set in your .env file
// ═══════════════════════════════════════════════════════════════════════
...
class TripDetectionService {
    ...
    // FIXED: reduced from 10 000 / 50 000 to safer values that won't
    // exhaust memory or cause DB timeouts on a single transaction
    ...
    static async reverseGeocode(latitude, longitude) {
        try {
            ...
            const params = {
                ...
                key: GOOGLE_MAPS_API_KEY,   // ✅ FIXED: env var, no hard-coded fallback
                language: 'en'
            };
            ...
            if (status === 'OVER_QUERY_LIMIT') {
                logger.warn('⚠️ Google Maps API quota exceeded');
                return { address: 'Location unavailable (rate limit)', status: 'failed', success: false };
            }

            if (status === 'REQUEST_DENIED') {
                logger.error('❌ Google Maps API request denied — check key restrictions');
            ...
            if (status === 'UNKNOWN_ERROR') {
                logger.warn('⚠️ Google Maps API unknown error — will retry later');
            ...
            logger.warn(`⚠️ Geocoding failed with status: ${status}`);
            return { address: 'Location unavailable', status: 'failed', success: false };

        } catch (error) {
            // Log the error type without leaking the API key or full URL
            if (error.code === 'ECONNABORTED') {
                logger.error('❌ Geocoding timeout — Google Maps API took too long');
            } else if (error.code === 'ENOTFOUND') {
                logger.error('❌ Geocoding DNS error — cannot reach maps.googleapis.com');
            } else if (error.code === 'ECONNREFUSED') {
                logger.error('❌ Geocoding connection refused — check network/firewall');
            } else {
        ...
        try {
            ...
            for (const mac of macs) {
                try {
                    ...
                    logger.error(`🔥 Error processing vehicle ${mac}:`, error.message);
                    errors++;
                }
            }
            ...
            logger.error('🔥 Fatal error in trip detection:', error.message);
    ...
    static async getVehiclesWithUnprocessedData() {
        try {
            ...
            logger.error('🔥 Error fetching vehicles with unprocessed data:', error.message);
            return [];
        }
    }
    ...
    static isValidGPSPoint(currentLoc, lastValidLoc, timeDiff) {
        ...
        if (timeDiff > this.MIN_JUMP_TIME_SECONDS) {
            ...
            if (speed > this.MAX_SPEED_KMPH) {
                logger.debug(`⚠️ GPS outlier rejected: ${speed.toFixed(1)} km/h (max: ${this.MAX_SPEED_KMPH})`);
                return false;
            }
        }

        if (distance > this.MAX_JUMP_DISTANCE_METERS && timeDiff < 10) {
            logger.debug(`⚠️ GPS outlier rejected: ${distance.toFixed(0)}m jump in ${timeDiff.toFixed(1)}s`);
            return false;
        }
    ...
    static isConsistentDirection(prevLoc, currentLoc, nextLoc) {
        ...
        if (angleDiff > this.DIRECTION_CHANGE_THRESHOLD) {
            ...
            if (distance > 50) {
                logger.debug(`⚠️ Inconsistent direction: ${angleDiff.toFixed(0)}° change`);
                return false;
            }
        }
    ...
    static async processVehicleLocations(macIdGps) {
        try {
            ...
            logger.info(`🔍 ${locationCount} unprocessed locations for ${macIdGps}`);
            ...
            if (totalTripsProcessed > 0) {
                logger.info(
                    `✅ ${vehicle.immatriculation}: ${totalTripsProcessed} trips processed` +
                    ` (${totalTripsMerged} merged)`
                );
            }
            ...
            logger.error(`🔥 Error processing vehicle ${macIdGps}:`, error.message);
            throw error;
        }
    }
    ...
    static async checkUserTripTracking(vehicleId) {
        try {
            ...
            logger.error(`🔥 Error checking trip tracking for vehicle ${vehicleId}:`, error.message);
    ...
    static async getOngoingTrip(vehicleId, macIdGps) {
        try {
            ...
            logger.error('🔥 Error fetching ongoing trip:', error.message);
            return null;
        }
    }
    ...
    static async detectAndProcessTrips(locations, vehicleId, macIdGps, ongoingTrip) {
                ...
                if (isMoving) {
                    ...
                    if (enoughPoints && enoughDistance) {
                        if (this.isCircularMovement(tripStartBuffer)) {
                            logger.warn('⚠️ Circular movement (parking lot drift) — NOT starting trip');
                        ...
                        logger.info(
                            `🚗 Starting new trip after ${consecutiveMovingPoints.length} moving points,` +
                        ...
                        lastValidPosition = { latitude: loc.latitude, longitude: loc.longitude, time: locTime };
        ...
        if (rejectedOutliers > 0 || rejectedDirectionErrors > 0) {
            logger.info(
                `🧹 GPS correction: ${rejectedOutliers} outliers rejected,` +
                ` ${rejectedDirectionErrors} direction errors rejected`
            );
        }
    ...
    static isRealMovement(currentLoc, lastRealPosition, speed) {
        ...
        if (speed < this.MAX_PARKED_SPEED) {
            ...
            if (distance < this.GPS_DRIFT_THRESHOLD_METERS) {
                logger.debug(`📍 GPS drift: ${distance.toFixed(1)}m at ${speed.toFixed(1)} km/h`);
                return false;
            }
        }
    ...
    static isCircularMovement(locations) {
        ...
        if (netDisplacement < this.PARKING_LOT_RADIUS_METERS) {
            logger.warn(
                `⚠️ Circular movement: ${locations.length} points,` +
                ` ${netDisplacement.toFixed(1)}m net displacement`
            );
            return true;
        }
        ...
        try {
            ...
            for (let i = 0; i < recentTrips.length - 1; i++) {
                ...
                if (gapMin > 0 && gapMin <= this.TRIP_MERGE_THRESHOLD_MINUTES) {
                    logger.info(`🔗 Merging trips ${older.id} and ${newer.id} (gap: ${gapMin.toFixed(1)} min)`);
            ...
            if (mergeCount > 0) logger.info(`✅ Merged ${mergeCount} trip pair(s)`);
            return mergeCount;

        } catch (error) {
            logger.error('🔥 Error merging trips:', error.message);
            return 0;
        }
    }
    ...
    static async mergeTwoTrips(trip1, trip2) {
        ...
        try {
            const [waypoints1, waypoints2] = await Promise.all([
                TripWaypoint.findAll({ where: { trip_id: trip1.id }, order: [['sequence_order', 'ASC']], raw: true, transaction }),
                TripWaypoint.findAll({ where: { trip_id: trip2.id }, order: [['sequence_order', 'ASC']], raw: true, transaction }),
            ]);
            ...
            await TripWaypoint.update(
                { trip_id: trip1.id, sequence_order: sequelize.literal(`sequence_order + ${offset}`) },
                { where: { trip_id: trip2.id }, transaction }
            );

            await Location.update({ trip_id: trip1.id }, { where: { trip_id: trip2.id }, transaction });
            await Trip.destroy({ where: { id: trip2.id }, transaction });

            await transaction.commit();
            logger.info(`✅ Merged trip ${trip2.id} into trip ${trip1.id}`);
            return true;
    ...
    static async createNewTrip(vehicleId, macIdGps, startLocation) {
        try {
            logger.info('🗺️ Geocoding trip start address...');
            ...
            logger.info(`✅ Created trip ${trip.id} — start: "${startAddressData.address}"`);
    ...
    static async updateOngoingTrip(currentTrip, endLocation, newWaypoints, locationIds) {
        ...
        try {
            if (newWaypoints.length > 0) {
                ...
                const waypointsToInsert   = correctedWaypoints.map(w => ({ ...w, trip_id: currentTrip.id }));

                // FIXED: always use chunked inserts — batch size now comes from the class constant
                ...
                logger.debug(`📍 Added ${correctedWaypoints.length} waypoints to trip ${currentTrip.id}`);
            }
            ...
            logger.info(`✅ Updated ongoing trip ${currentTrip.id} — ${allWaypoints.length} waypoints,` +
                ` ${Math.round(metrics.durationMinutes)} min, ${metrics.totalDistanceKm.toFixed(2)} km`);

            return true;

        } catch (error) {
            await transaction.rollback();
            logger.error(`🔥 Error updating ongoing trip ${currentTrip.id}:`, error.message);
            return false;
        }
    }
    ...
    static async finalizeTrip(currentTrip, endLocation, newWaypoints, locationIds) {
        ...
        try {
            if (newWaypoints.length > 0) {
                ...
                // FIXED: always chunked — no conditional needed
                for (let i = 0; i < waypointsToInsert.length; i += this.WAYPOINT_BATCH_SIZE) {
                    await TripWaypoint.bulkCreate(
                        waypointsToInsert.slice(i, i + this.WAYPOINT_BATCH_SIZE),
                        { transaction }
            ...
            logger.info(`📍 Trip ${currentTrip.id}: ${allWaypoints.length} waypoints before simplification`);
            ...
            if (simplifiedWaypoints.length < allWaypoints.length) {
                ...
                logger.info(`✅ Path simplified: ${allWaypoints.length} → ${simplifiedWaypoints.length} waypoints (${reduction}% reduction)`);
                ...
                // FIXED: chunked insert for simplified waypoints too
            ...
            // ── Geocode end address ────────────────────────────────────
            logger.info('🗺️ Geocoding trip end address...');
            ...
            logger.info(
                `✅ Trip ${currentTrip.id} finalized — end: "${endAddressData.address}"` +
            ...
            await transaction.rollback();
            logger.error(`🔥 Error finalizing trip ${currentTrip.id}:`, error.message);
            return false;
        }
    }
